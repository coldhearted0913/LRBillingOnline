...
export async function GET(request: NextRequest) {
  try {
    ...
    // Call the weekly notifications endpoint
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 
                    process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 
                    'http://localhost:3000';
    
    const response = await fetch(`${baseUrl}/api/whatsapp/weekly-notifications`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({
      success: true,
      message: 'Weekly notifications triggered successfully',
      ...data,
    });
  } catch (error: any) {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error?.message || 'Failed to trigger weekly notifications',
      },
      { status: 500 }
    );
  }
